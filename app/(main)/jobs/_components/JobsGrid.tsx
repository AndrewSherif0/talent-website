...
import { MapPin, Users, Calendar, Banknote, MessageCircle, BadgeCheck } from "lucide-react";
...
function JobCard({ job, dark, lang, index }: { job: JobPost; dark: boolean; lang: "ar" | "en"; index: number }) {
  const ar     = lang === "ar";
    ...
    >
      ...
      <div style={{ padding: "12px 16px 16px" }}>
        ...
        >
          ...
          {ar ? "تواصل مع البراند" : "Contact Brand"}
        </button>
      </div>
    </motion.div>
  );
...
export default function JobsGrid({ dark, lang, jobs }: Props) {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 20 }}>
      {jobs.map((job, i) => (
        <JobCard key={job.id} job={job} dark={dark} lang={lang} index={i} />
      ))}
    </div>
  );
}
