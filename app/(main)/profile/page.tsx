...
export default function MyProfilePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { profile: prof, talentProfile: talentProf } = await res.json();
  ...
  const inp: React.CSSProperties = {
    width: "100%", padding: "10px 14px", backgroundColor: INP,
  ...
  if (status === "loading") return (
    <div style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Cairo', sans-serif" }}>
      <div style={{ textAlign: "center" }}>
        <div style={{ width: "40px", height: "40px", borderRadius: "50%", border: `3px solid ${BORDER}`, borderTopColor: GOLD, animation: "spin 0.8s linear infinite", margin: "0 auto 16px" }} />
        <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
        <p style={{ color: MUTED, fontSize: "14px" }}>{tx.loading}</p>
      </div>
    </div>
  );
  ...
  if (status === "none") return (
    <div style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <p style={{ color: MUTED, fontFamily: "'Cairo', sans-serif" }}>{tx.noProfile}</p>
    </div>
  );

  const sl = tp?.social_links ?? {};
  const createdAt = profile.created_at ? new Date(profile.created_at).toLocaleDateString(lang === "ar" ? "ar-EG" : "en-US", { month: "long", year: "numeric" }) : "";
